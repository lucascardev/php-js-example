...
function formSender(f, campo) { 
	...
	if(!acao) { 
		alert("erro: o valor action do formulário nao foi definido"); 
	} 
	var send = new Array(); 
...
function Ajax(url, campo, query) {
	...
	if (window.XMLHttpRequest) {  
		...
		alert("Seu navegador nativo não suporta XMLHttpRequest.");  
		return;  
	}
	req.open("GET", url+"_ajax.php?"+query, false);
...
function nextDay(day){
	var nextDay = moment(day, "DD/MM/YYYY").add(1, 'days').format("DD/MM/YYYY");
	return nextDay;
}
...
function JsPanelAgendaMarcacaoCreate(panelId, contentId, horario, date) {
	jsPanel.create({
        id: panelId,
		...
		resizeit: false,
		theme: {
        bgPanel: 'url("imagens/background.png") right bottom no-repeat',
        bgContent: '#fff',
        colorHeader: '#fff',
        colorContent: `#${jsPanel.colorNames.gray700}`,
        },
        headerControls: 'closeonly sm',
		callback: panel => panel.setHeaderTitle(date+'-'+horario),
      }).content.id = contentId;
}


function JsPanelAtendenteQrCode() {
	jsPanel.create({
		id: 'QR-Code-Atendente',
		onclosed: function(panel, closedByUser) {
			clearInterval(intervalQr);
        },
		headerTitle: 'Atendente Virtual QR-Code session',
		animateIn: 'jsPanelFadeIn',
		animateOut: 'jsPanelFadeOut',
		resizeit: false,
		contentAjax: {
        url: 'whatsappBot/qrcode_ajax.php'
        },
		contentSize: '400 400',
		theme: {
        bgPanel: 'url("imagens/background.png") right bottom no-repeat',
